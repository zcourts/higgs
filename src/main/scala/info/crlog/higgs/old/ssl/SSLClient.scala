...
import com.fillta.https.SSLConfiguration
import com.fillta.https.SSLContextFactory
...
import info.crlog.higgs.agents.http.{EventProcessor, HttpClientHandler}
...
class SSLClient {
  ...
  def execute {
    ...
    try {
      b.group(new NioEventLoopGroup).channel(classOf[NioSocketChannel]).remoteAddress(host, port).handler(new ChannelInitializer[SocketChannel] {
