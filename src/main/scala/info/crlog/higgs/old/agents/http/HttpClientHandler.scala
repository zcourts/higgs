 ...
 */
class HttpClientHandler(override val future: EventProcessor) extends ClientHandler[AnyRef](future) {
  ...
  override def messageReceived(ctx: ChannelHandlerContext, msg: AnyRef) {
    if (future.channel == None) {
      //if the events doesn't have a channel yet then assign this
