...
import socket.nio.{NioEventLoopGroup, NioServerSocketChannel}
...
class MsgpackClient(host: String, port: Int) {
  ...
  var handler = new MsgpackClientHandler(listeners)
  var channelfuture: ChannelFuture = null
  var connected = false
  val unsentMessages = mutable.Queue.empty[Interaction]
  /**
   ...
   */
  def send[T <: Interaction](msg: T): MsgpackClient = {
  ...
  def enqueueMessage[T <: Interaction](msg: T) = {
    unsentMessages += msg
    if (connected) {
      flushMessageQueue()
    }
  ...
  def newhandler() {
    ...
    handler = new MsgpackClientHandler(listeners)
    exlisteners foreach {
      case l => {
