...
class WebSocketServerHandler[T <: Interaction](
                                                ...
                                                textClients: mutable.Map[String, ListBuffer[ChannelHandlerContext]],
                                                textListeners: mutable.Map[String, ListBuffer[(TextMessage, ChannelHandlerContext) => Unit]]
  ...
  def unmarshalText(data: String, ctx: ChannelHandlerContext, isHTTP: Boolean = false) {
    val msg = TextMessage(data)
    val listeners = ListBuffer.empty[(TextMessage, ChannelHandlerContext) => Unit]
    textListeners.get("") match {
      case None =>
      case Some(subList) => listeners ++= subList
    }
    textListeners.get(msg.topic) match {
      case None =>
      case Some(subList) => listeners ++= subList
    }
    listeners foreach {
  ...
  private def handleWebSocketFrame(ctx: ChannelHandlerContext, frame: WebSocketFrame) {
    if (frame.isInstanceOf[CloseWebSocketFrame]) {
      ...
      unmarshalText(frame.asInstanceOf[TextWebSocketFrame].getText, ctx)
      return
  ...
  private def handleHttpRequest(ctx: ChannelHandlerContext, req: HttpRequest) {
    if (req.getUri.startsWith("/poll")) {
      unmarshalText(StringUtil.getString(req.getContent.array()), ctx, true)
      return
    }
