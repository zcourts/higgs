...
class WebSocketServer(host: String, port: Int) {
  ...
  val textListeners = mutable.Map.empty[String, ListBuffer[(TextMessage, ChannelHandlerContext) => Unit]]
  val textClients = mutable.Map.empty[String, ListBuffer[ChannelHandlerContext]]
  val msgpack = new MessagePack()

  var channel: Channel = null

  def emit(topic: String, msg: String) {
    val subscribers = ListBuffer.empty[ChannelHandlerContext]
    //a subscriber with empty string as topic must get ALL messages
    if (!topic.isEmpty) {
      textClients get ("") match {
        case None => //nothing listening for all messages
        case Some(subs) => subscribers ++= subs
      }
    }
    //get actual subscribers to this topic
    textClients get (topic) match {
      case None => log.warn("No subscribers for topic (%s)".format(topic))
      case Some(subList) => {
        subscribers ++= subList
      }
    }
    //now emit all subscribers
    subscribers.foreach {
      case ctx => {
        if (ctx.channel().isOpen) {
          ...
          subscribers -= ctx
          log.warn("Text Channel found matching subscription but the channel is not open, its been removed")
        }
      }
    }
  }

  def listen(topic: String, handler: (TextMessage, ChannelHandlerContext) => Unit) {
    textListeners.getOrElseUpdate(topic, ListBuffer.empty) += handler
  }
  ...
  def bind() {
    ...
    b.group(new NioEventLoopGroup, new NioEventLoopGroup)
      ...
      def initChannel(ch: SocketChannel) {
        ...
        pipeline.addLast("handler", new WebSocketServerHandler(clients, listeners, textClients, textListeners))
      }
    })
