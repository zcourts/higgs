...
import info.crlog.higgs.{Request, EventProcessor, Client}
...
class HttpClient(var USER_AGENT: String = "Mozilla/5.0 (compatible; HiggsBoson/0.0.1; +https://github.com/zcourts/higgs)")
  ...
  def POST(url: URL, data: Map[String, Any]): EventProcessor = {
   ...
   * @param response The response events which will be given the response of this request
   * @param shutdown  If true then the request's bootstrap is shutdown
   *                  </pre>
   */
  def makeRequest(response: EventProcessor,
                  method: HttpMethod,
                  url: URL,
                  cookies: Map[String, String] = Map.empty[String, String],
                  block: Boolean = true,
