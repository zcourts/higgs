...
import io.higgs.reflect.PackageScanner;
...
public abstract class RPCServer<OM, IM, SM> extends HiggsServer<String, OM, IM, SM> {
        ...
        for (Constructor constructor : klass.getConstructors()) {
            if (!registered && //not registered yet
                ...
                try {
                    ...
                    log.warn(String.format("%s ignored. Cannot access constructor, is it private?",
                            klass.getName()), e);
    ...
    public void doRegister(Class<?> klass, final Object instance, final Method method) {
        ...
        if (listening(methodName)) {
            ...
            log.warn(String.format("Method name %s is already registered, ignored, overloaded methods " +
                    "not yet supported!", methodName));
            return;
        }
        ...
        log.info(String.format("REGISTERED > %1$-20s | %2$-20s | %3$-5s", method.getName(), methodName,
                method.getReturnType().getName()));
    }

    public void processIncomingRequest(final ChannelMessage<IM> request, Method method, Object instance,
                                       String methodName) {
    ...
    protected Object[] extractIncomingRequestParameters(Class<?>[] argTypes, Object[] args, ChannelMessage<IM> request,
                                                        String methodname) {
     ...
     * @return
     */
    protected abstract OM newResponse(String methodName, ChannelMessage<IM> request, Optional<Object> returns,
                                      Optional<Throwable> error);

    private void logDetailedFailMessage(Method method, String methodName, Object[] args, Class<?>[] argTypes,
                                        Throwable e) {
        String expected = "[";
